...
| `Jack.setShiftHeld(held)` / `Jack.setPointerHoverPosition(x, y)` (statici) | Alimentano lo stato usato dall'anteprima "jack pieno" (vedi sotto) — normalmente chiamati dal controller di interazione in risposta a `keydown`/`keyup`/`pointermove`, non da codice applicativo |
| `Jack.setDragActive(active)` (statico) | Segnala che un drag che potrebbe tentare di collegarsi a un jack è iniziato/terminato (vedi sotto) |
...
Oltre ad essere un drop target, `Jack` espone un'API per avviare, far avanzare e concludere la creazione di un nuovo cavo — nel comportamento standard (`StandardInteractionController`): click destro, oppure click sinistro + <kbd>Shift</kbd>, su un jack con capacità residua crea immediatamente un nuovo `<cavi-wire>` — un `<cavi-plug>` viene subito agganciato a quel jack, l'altro segue il cursore.
...
- **`finishCableSession`**: se un jack compatibile è in prossimità, il plug libero si aggancia esattamente come un normale rilascio di `Plug` (`fixed = true`, `attach()`, `plugged`). Altrimenti il cavo resta agganciato all'origine con il capo libero lasciato pendente — `node.fixed = false`, identico a un `Plug` normale rilasciato lontano da ogni jack (libero di muoversi sotto la fisica, e già completamente ri-trascinabile da solo dato che è un vero `<cavi-plug>`). `cancelCableSession` produce lo stesso esito senza tentare alcuno snap.
- **Un jack pieno mostra un cursore "vietato"**: se la posizione nota tramite `Jack.setPointerHoverPosition()` si trova entro `20`px dal centro di un jack che ha raggiunto `max-plugs` (`!canAcceptMore()`) **mentre `Jack.setShiftHeld(true)` oppure è in corso un drag segnalato con `Jack.setDragActive(true)`**, l'host riceve `cursor: not-allowed` (stile inline, vince sempre sulla regola `:host { cursor: crosshair }`) e la classe `full-class`. Il rilevamento hover è basato sulla distanza dall'ultima posizione nota (uno stato statico condiviso da tutti i jack, alimentato dall'esterno), **non** su `pointerenter`/`pointerleave` nativi sul jack: un jack con almeno un plug agganciato ha quel `<cavi-plug>` posizionato esattamente al suo centro con `z-index` più alto, quindi lo occlude e impedirebbe agli eventi di hover nativi di raggiungerlo mai.

  La condizione "Shift premuto" e la condizione "un drag è in corso" sono **tenute separate apposta**: lo stato alimentato da `Jack.setShiftHeld()` è sempre valido anche prima che qualunque drag inizi (l'anteprima hover pre-drag); quello alimentato da `Jack.setDragActive()` (un contatore interno, non un booleano, così due drag sovrapposti — es. multi-touch — non si azzerano a vicenda) è invece pensato per restare attivo per l'intera durata di un gesto anche se Shift viene rilasciato a metà — necessario perché Shift/right-click servono solo ad *avviare* il drag di creazione cavo, non a mantenerlo: una volta partito, il cavo continua a seguire la posizione anche rilasciando Shift (comportamento voluto, per non dover tenere premuto un tasto per tutta la durata del trascinamento). Senza `setDragActive`, l'anteprima "vietato" sparirebbe non appena si rilascia Shift a metà drag, anche continuando a trascinare il capo libero verso un jack pieno.

  Lo stato si aggiorna anche senza una nuova posizione quando il jack si riempie/libera (`attach`/`detach`) o quando uno dei due drag inizia/termina, mentre il jack è già sotto osservazione.
- **Un jack pieno ha sempre `at-capacity-class`**: a differenza di `full-class`, questa classe è incondizionata — riflette semplicemente `!canAcceptMore()`, indipendentemente da hover o Shift, quindi resta utilizzabile per uno stile "pieno" persistente (es. un bordo o un'icona) visibile anche senza interagire col jack.
...
**`StandardInteractionController`** (`src/interaction.ts`) è l'implementazione standard, e riproduce esattamente il comportamento di trascinamento descritto in questo documento: un'unica istanza installa pochi listener a livello di `document`/`window` (`pointerdown`, `contextmenu`, `keydown`/`keyup`, `blur`, `pointermove`) e, per ogni gesto riconosciuto, pilota Jack/Plug esclusivamente tramite le loro API pubbliche descritte sopra — mai toccando stato interno.
...
- **Modalità `hold` vs `click-to-carry`** (`Cavi.setDragMode('hold' | 'click')`, opzione globale a livello di mondo, non per singolo elemento):
  - **`hold`** (default): premi-trascina-rilascia classico. Il `pointerdown` avvia il gesto e chiama `setPointerCapture` sull'elemento target, così tutti gli eventi pointer successivi per quel puntatore restano instradati lì anche se il cursore esce dai suoi confini; `pointerup`/`pointercancel` lo concludono. **Mentre il pulsante resta premuto, lo scroll nativo del browser — in particolare il gesto a due dita del touchpad — smette tipicamente di essere riconosciuto** (limitazione comune di molti driver, non specifica di questa libreria).
  - **`click`**: un click (senza tenerlo premuto) avvia il gesto — un listener `pointermove` a livello di `document` segue la posizione **senza che nessun pulsante resti premuto**, così lo scroll nativo funziona per tutta la durata come se non si stesse trascinando nulla. Un secondo click (pulsante primario, ovunque avvenga) conclude il gesto; un click con pulsante non primario viene ignorato.
  - **Eccezione touch**: `click-to-carry` si applica solo a mouse e pen (`pointerType !== 'touch'`) — il touch usa sempre `hold`, perché il trascinamento naturale col dito non ha il conflitto con lo scroll descritto sopra (già disabilitato durante il drag da `touch-action: none`).
- **Trigger della creazione cavo**: `pointerdown` con `button === 2` (tasto destro) oppure `button === 0 && shiftKey`, su un `<cavi-jack>` — chiama `jack.createCable()`. Il menu contestuale nativo viene sempre soppresso su un jack (`contextmenu` → `preventDefault()`).
- **Occlusione jack↔plug**: un plug agganciato a un jack è fissato esattamente al centro del jack con `z-index` più alto — in un browser reale un click su quel punto risolve sempre sul plug, mai sul jack sottostante. Il controller riconosce l'evento tramite `event.composedPath()` (che attraversa correttamente lo Shadow DOM) e, se il target risolto è un `<cavi-plug>` agganciato **e** il gesto è right-click o Shift+click, lo tratta come se fosse avvenuto sul suo `.jack`, avviando la creazione di un nuovo cavo da lì invece di trascinare il plug. Per lo stesso motivo il menu contestuale nativo viene soppresso anche su un plug agganciato.
- **Trascinamento di un `Plug` esistente**: un `pointerdown` col tasto primario e senza Shift (e non intercettato dal caso di occlusione sopra) chiama `plug.beginDrag()`. Shift è riservato al gesto di creazione cavo — un plug non viene trascinato mentre è premuto.
- **Tracciamento Shift/hover**: `keydown`/`keyup` su `Shift` alimentano `Jack.setShiftHeld()`; `blur` sulla finestra lo azzera (un `keyup` può non arrivare se il focus lascia la pagina, es. alt-tab); ogni `pointermove` a livello di `document` alimenta `Jack.setPointerHoverPosition()`.
- **`detach()`**: rimuove tutti i listener e azzera lo stato Shift/hover esposto tramite i setter di `Jack`, così nessun jack resta bloccato in un'anteprima "vietato" residua.
